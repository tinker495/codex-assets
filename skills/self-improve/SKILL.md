...
| Planning | Hypothesis → structured plan | oh-my-Codex:planner | opus |
| Architecture Review | 6-point plan review | oh-my-Codex:architect | opus |
| Critic Review | Harness rule enforcement | oh-my-Codex:critic | opus |
| Execution | Implement plan + run benchmark | oh-my-Codex:executor | opus |
| Git Operations | Atomic merge/tag/PR | oh-my-Codex:git-master | sonnet |
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `oh-my-Codex:git-master`:
  ```
...
Spawn N `oh-my-Codex:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `oh-my-Codex:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `oh-my-Codex:critic` with the plan + harness rules:
...
For each approved plan, spawn `oh-my-Codex:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `oh-my-Codex:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/oh-my-Codex:cancel` for clean state cleanup
